...
- **M4**: 雑記向け Neocities 週次 HTML アップロード（必要なら月次も。`publisher/`）
- **M5**: Planet ページ公開 — **JSON 生成（build_feed）✅** / **Neocities クライアント（`neocities/planet/index.html` + `planet-app.js`、手動アップロード・アイコン配置は README 参照）✅**
- **M6**: cron 自動実行（最小追記）
