...
**前提**: 自動投入は `./venv/bin/python -m summarizer.generate` — **週** `--period week --date YYYY-Www`、**月** `--period month --date YYYY-MM`（venv 必須: `psycopg2`）。バッチ・Neocities は未実装。
...
| `/summaries` | **週・月のみ**一覧 + 週行に **7 日分の日次へのリンク**（`/?view=day&date=`）+ Neocities 公開トグル |
| カレンダー週/月/日 | `#summary-panel` で `GET /api/summary`（週・月・**日**） |
...
- **`period_type`**: `'weekly'` / `'monthly'` / **`'daily'`**（日次は週次階層生成時に保存。**一覧 `/summaries` では週・月のみ表示**し、日次はカレンダー日ビューと週行のリンクから参照）。詳細は **`docs/summary_daily_and_dashboard.md`**。
...
3. **`GET /api/summary`**（実装済み）  
   - クエリ: `period=day|week|month|year`、`date` は日 `YYYY-MM-DD`、週 `YYYY-Www`、月 `YYYY-MM`、年は西暦  
   - 日・週・月: 該当 1 件 or `summary: null`。年: `summaries` 配列（月次のみ）  
