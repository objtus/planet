...
**前提**: 現状 `summarizer/` は未実装。行の投入は手動 SQL・将来の `generate.py`・検証用スクリプトで行う想定。
...
| `/summaries` | 全件 `ORDER BY period_start DESC` で一覧表示。`is_published` はバッジ表示のみ。**トグルなし** |
| カレンダー週/月/年 | タイムライン＋統計のみ。**サマリー未取得** |
| 年ビュー | 投稿一覧は省略済み。`detail-title` 下に**月次サマリーのインデックス**を置くと `design.md` の「月別サマリー一覧」と整合 |
...
3. **（任意）`GET /api/summary`**  
   - クエリ: `period=week|month&date=...`（週は `YYYY-Www`、月は `YYYY-MM`）  
   - 戻り: 該当 1 行の `id`, `content`, `is_published` 等、なければ `null` / 404  
   - カレンダーからの取得を **1 本にまとめ**、SQL を `app.py` 内で共通化しやすい
